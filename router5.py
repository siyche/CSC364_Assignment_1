...
def main():
    router_num = 5
    forwarding_table = load_forwarding_table(f'router_5_table.csv')
    ...
    with open('packets.csv', 'r') as f:
        ...
        for row in reader:
            ...
            if packet:
                print(f"Sending packet: {packet['payload']} TTL={packet['ttl']} to Router 6")
                with open(f"sent_by_router_{router_num}.txt", 'a') as out_file:
                    out_file.write(f"{packet['payload']} TTL={packet['ttl']} to Router 6\n")
                send_packet('127.0.0.1', 8006, packet, packet['ttl'])
                time.sleep(1)
