...
def main():
    router_num = 6
    forwarding_table = load_forwarding_table(f'router_6_table.csv')
    ...
    with open('packets.csv', 'r') as f:
        ...
        for row in reader:
            ...
            if packet:
                print(f"Sending packet: {packet['payload']} TTL={packet['ttl']} to Router 7")
                with open(f"sent_by_router_{router_num}.txt", 'a') as out_file:
                    out_file.write(f"{packet['payload']} TTL={packet['ttl']} to Router 7\n")
                send_packet('127.0.0.1', 8007, packet, packet['ttl'])
                time.sleep(1)
